...
from obsidian_sync.file_utils import check_is_srs_file, move_file_to_system_trash, check_files_are_identical
...
class ObsidianVault:
    ...
    def attachment_paths_from_file_text(self, file_text: str, note_path: Path) -> Dict[str, Path]:
        ...
        attachment_paths = {
            path_string: self._resolve_attachment_path(attachment_path_in_note=Path(path_string), note_path=note_path)
            for path_string, optional_part in
            re.findall(markdown_attachment_pattern, file_text, re.DOTALL)
        }
        return attachment_paths

    def ensure_attachment_is_in_obsidian(self, attachment: LinkedAttachment) -> Tuple[str, Path]:
        obsidian_attachment_path = self._obsidian_config.srs_attachments_folder / attachment.path.name
        obsidian_file_text_path = str(obsidian_attachment_path.relative_to(self._obsidian_config.vault_folder))

        if not check_files_are_identical(first=attachment.path, second=obsidian_attachment_path):
    ...
    def _resolve_attachment_path(self, attachment_path_in_note: Path, note_path: Path) -> Path:
        ...
        if attachment_path_in_note.name == str(attachment_path_in_note):
            if (note_path / attachment_path_in_note.name).exists():
                attachment_path = note_path / attachment_path_in_note
            else:
                attachment_path = next(
                    self._addon_config.obsidian_vault_path.rglob(pattern=str(attachment_path_in_note))
                )
        else:  # path relative to vault directory
            attachment_path = self._addon_config.obsidian_vault_path / attachment_path_in_note
